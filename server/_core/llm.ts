...
const assertApiKey = () => {
  if (!ENV.forgeApiKey) {
    throw new Error(
      "LLM API Key 未配置：请在环境变量中设置 BUILT_IN_FORGE_API_KEY 或 OPENAI_API_KEY"
    );
  }
};
...
export async function invokeLLM(params: InvokeParams): Promise<InvokeResult> {
  ...
  const maxOut =
    params.max_tokens ?? params.maxTokens ?? 32768;
  ...
  // OpenAI 新模型与部分网关要求 max_completion_tokens，不接受 max_tokens
